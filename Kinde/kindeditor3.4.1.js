...
KE.setting = {
	...
	items : [
		...
		'flash', 'media', 'table', 'hr', 'emoticons', 'link', 'unlink', 'about','ke_upfile'
	],
	colorTable : [
...
KE.plugin['ke_upfile'] = {
	...
	click : function(id) {
		...
		this.dialog = new KE.dialog({
			id : id,
			cmd : 'ke_upfile',
			file : 'image/file.html?id=' + id + '&ver=' + KE.version,
			width : 400,
			height : 100,
			loadingMode : true,
			title : KE.lang['ke_upfile'],
			yesButton : KE.lang['yes'],
			noButton : KE.lang['no']
		});
		this.dialog.show();
	...
	insert : function(id, url, title, width, height, border, align) {
		...
		html += 'alt="' + title + '" ';
		html += 'border="' + border + '" />';
		var html='<a href="'+url+'" title="点击下载" target="_blank">';
		html+='<img src="'+KE.scriptPath+'skins/common/b_upfile.gif" border="0" style="vertical-align:middle" />&nbsp;'+title+'</a>';
		KE.util.insertHtml(id, html);
		KE.layout.hide(id);
		KE.util.focus(id);
	}
