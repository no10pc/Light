...
namespace light.Common
{
    ...
    public class Utility
    {
        ...
        public static bool IsCompriseStr(string str, string stringarray, string strsplit)
        {
            ...
            str = str.ToLower();
            string[] stringArray = Utility.SplitString(stringarray.ToLower(), strsplit);
        ...
        public static decimal cutipconfrim(string ip)
        {
            long[] m = { 1, 255, 65025, 16581375 };
            string[] ipconfrim = ip.Split('.');
            int len = ip.Split('.').Length;
            decimal ipnum = 0;
            for (int i = 0; i < len; i++)
            {
                ipnum += long.Parse(ipconfrim[i]) * m[len - (i + 1)];
            }
            return ipnum;
        }
        /// <summary>
        ...
        public static string[] FindNoUTF8File(string Path)
        {
            ...
            */
            ...
            return Utility.SplitString(filelist.ToString(), "\r\n");
        ...
        public static bool InIPArray(string ip, string[] iparray)
        {
            ...
            string[] userip = Utility.SplitString(ip, @".");
            for (int ipIndex = 0; ipIndex < iparray.Length; ipIndex++)
            {
                string[] tmpip = Utility.SplitString(iparray[ipIndex], @".");
                int r = 0;
        ...
        public static HttpCookie GetUserCookie(string strName)
        {
            if (HttpContext.Current.Request.Cookies != null && HttpContext.Current.Request.Cookies[strName] != null && HttpContext.Current.Request.Cookies[strName]["uid"] != null)
            {
        ...
        /// <param name="strName">名称</param>
        /// <returns>cookie值</returns>
        public static void ClearUserCookie(string strName)
        {
            //HttpContext.Current.Request.Cookies[strName].Expires = DateTime.Now.AddYears(-1);
            HttpContext.Current.Request.Cookies.Remove(strName);
            HttpCookie cookie = new HttpCookie(strName);
            cookie.Values.Clear();
            cookie.Expires = DateTime.Now.AddYears(-1);
            cookie.Domain = ".wzcbd.com";
            HttpContext.Current.Response.AppendCookie(cookie);

            HttpContext.Current.Request.Cookies.Remove("dntadmin");
            //HttpCookie cookiednt = new HttpCookie("dntadmin");
            //cookiednt.Values.Clear();
            //cookiednt.Expires = DateTime.Now.AddYears(-1);
            //cookiednt.Domain = "wzcbd.com";
            //HttpContext.Current.Response.AppendCookie(cookiednt);
        }
        ...
        /// <returns></returns>
        public static string GetTextFromHTMLS(string source)
        {
                        ...
                        if (strs != "")
                            ...
                            switch (ruletype.Trim().ToLower())
                            {
                                ...
                                case "timesect":
                                    ...
                                    if (Utility.IsTime(splitetime[1].ToString()) == false || Utility.IsTime(splitetime[0].ToString()) == false)
                                        throw new Exception();
                                    break;
        ...
        public static string paraDateTime(DateTime addtime)
        {
            TimeSpan span = new TimeSpan();
            span = DateTime.Now - addtime;
        ...
        /// <returns></returns>
        public static string getshopothefieldrname(int typeid)
        {
            string fieldname = "pkid";
            switch (typeid)
            {
                case 0:
                    fieldname = "store_GongGao";
                    break;
                case 1:
                    fieldname = "store_Message";
                    break;
                case 2:
                    fieldname = "store_XuZhi";
                    break;
                case 3:
                    fieldname = "store_BaoJia";
                    break;
                case 4:
                    fieldname = "store_JieShao";
                    break;
            };
            return fieldname;
        }
        /// <summary>
        /// 店铺LOGO,BANNER上传方法(与产品多图文件夹规则不同)
        /// </summary>
        /// <param name="oFile">HttpPostedFile</param>
        /// <param name="PhysicsPath">物理路径F:\uppic\UserFiles</param>
        /// <param name="UrlPath"></param>
        /// <param name="userId"></param>
        /// <returns></returns>
        public static string upload(HttpPostedFile oFile, string PhysicsPath, string storeid)
        {
            bool Synchro = bool.Parse(System.Configuration.ConfigurationManager.AppSettings["FCKeditor:Synchro"]); ;
            string PostUrl = System.Configuration.ConfigurationManager.AppSettings["FCKeditor:PostUrl"];
            string UserFilesPath = System.Configuration.ConfigurationManager.AppSettings["FCKeditor:UserFilesPath"];
            string PicSiteUrl = System.Configuration.ConfigurationManager.AppSettings["FCKeditor:PicSiteUrl"];
            ...
            if (!CheckFileExtends(GetFileExtends(oFile.FileName),"jpg,gif"))
            {
                return "文件格式不正确";
            }
            string dateTimePath = MakeFilePath();
            //生成随机文件名 
            string sFileName = MakeFileNameString() + System.IO.Path.GetExtension(oFile.FileName);
            string tempFileName = "s_" + sFileName;

            CreateFolder(UserFilesPath + PhysicsPath + "\\" + storeid + "\\" + dateTimePath);

            string sFilePath = System.IO.Path.Combine(UserFilesPath + PhysicsPath + "\\" + storeid + "\\" + dateTimePath + "\\", sFileName);
            oFile.SaveAs(sFilePath);

            if (Synchro)
            {
                WebClientPostFile(PostUrl + PicSiteUrl.Substring(PicSiteUrl.LastIndexOf("/") + 1) + "\\" + PhysicsPath + "\\" + storeid + "\\" + dateTimePath, sFilePath);
            }

            if (!PicSiteUrl.EndsWith("/"))
                PicSiteUrl += "/";
            string sFileUrl = PicSiteUrl + PhysicsPath + "/" + storeid + "/" + dateTimePath + "/" + sFileName;
        ...
        /// <returns></returns>
        public static string uploadSilder(HttpPostedFile oFile, string PhysicsPath, string storeid)
        {
            string[] type = { "Big", "Small", "Middle" };
            bool Synchro = bool.Parse(System.Configuration.ConfigurationManager.AppSettings["FCKeditor:Synchro"]); ;
            string PostUrl = System.Configuration.ConfigurationManager.AppSettings["FCKeditor:PostUrl"];
            string UserFilesPath = "g:\\uppic\\";
            string PicSiteUrl = System.Configuration.ConfigurationManager.AppSettings["FCKeditor:PicSiteUrl"];
            // Check if the file has been correctly uploaded
            ...
            if (!CheckFileExtends(GetFileExtends(oFile.FileName),"jpg"))
            {
                return "文件格式不正确";
            }
            ...
            {
                CreateFolder(UserFilesPath + PhysicsPath + "\\" + storeid + "\\" + str + "\\" + dateTimePath);
                CreateFolder(UserFilesPath + PhysicsPath + "\\" + storeid + "\\" + str + "\\" + dateTimePath);
                CreateFolder(UserFilesPath + PhysicsPath + "\\" + storeid + "\\" + str + "\\" + dateTimePath);
            }

            string[] sFilePath = { System.IO.Path.Combine(UserFilesPath + PhysicsPath + "\\" + storeid + "\\"+type[0]+"\\" + dateTimePath + "\\", sFileName), 
                                   System.IO.Path.Combine(UserFilesPath + PhysicsPath + "\\" + storeid + "\\"+type[1]+"\\" + dateTimePath + "\\", sFileName),
                                   System.IO.Path.Combine(UserFilesPath + PhysicsPath + "\\" + storeid + "\\"+type[2]+"\\" + dateTimePath + "\\", sFileName)                                
                                 };

            oFile.SaveAs(sFilePath[0]);

            MakeThumbnailImage(sFilePath[0], sFilePath[1], 58, 58, ImageMode.HW);
            MakeThumbnailImage(sFilePath[0], sFilePath[2], 310, 310, ImageMode.HW);


            if (Synchro)
            {
                WebClientPostFile(PostUrl + PicSiteUrl.Substring(PicSiteUrl.LastIndexOf("/") + 1) + "\\" + PhysicsPath + "\\" + storeid + "\\" + type[0] + "\\" + dateTimePath, sFilePath[0]);
                WebClientPostFile(PostUrl + PicSiteUrl.Substring(PicSiteUrl.LastIndexOf("/") + 1) + "\\" + PhysicsPath + "\\" + storeid + "\\" + type[1] + "\\" + dateTimePath, sFilePath[1]);
                WebClientPostFile(PostUrl + PicSiteUrl.Substring(PicSiteUrl.LastIndexOf("/") + 1) + "\\" + PhysicsPath + "\\" + storeid + "\\" + type[2] + "\\" + dateTimePath, sFilePath[2]);
            }
            return sFileName;
        }
        /// <summary>
        ...
        /// <returns></returns>
        private static string GetRandom(int MaxValue)
        {
        ...
        public static string CreateFileName()
        {
            ...
            strMonth = dt.Month.ToString("00");
            strDay = dt.Day.ToString("00");
            strHour = dt.Hour.ToString("00");
            strMinute = dt.Minute.ToString("00");
            strSecond = dt.Second.ToString("00");
            strMillisecond = dt.Millisecond.ToString();

            strTemp = strYear + strMonth + strDay + "_" + strHour + strMinute + strSecond + "_" + strMillisecond + "_" + GetRandom(max);
        ...
        /// <returns></returns>
        /// 
        public static string MakeFileNameString()
        {
        ...
        /// 扩展名检测
        /// </summary>
        /// <param name="fileExtends">扩展名</param>
        /// <param name="fileType">列表"jpg,gif,swf"</param>
        /// <returns></returns>
        public static bool CheckFileExtends(string fileExtends,string fileType)
        {
            ...
            {
                if (names[i].ToLower() == fileExtends)
                {
                    return true;
                }
            }
        ...
        public static bool CheckFileType(string filePath)
        {
            ...
            string str = reader.ReadByte().ToString() + reader.ReadByte().ToString();
            input.Close();
            reader.Close();
            return (str == "255216");
        ...
        public static string GetRemoveFile(string path)
        {
            string Value = "";
            int i = 0;
